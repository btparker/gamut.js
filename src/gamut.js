...
(function(_this){
	...
	_this['gamut'] = function( fileData ){
		...
		function imageHistogram(){
			...
			var red, green, blue, brightness;

			return {
				init: function(){
					// For rgb color imageHistogram
					this.red = Array.apply(null, Array(256)).map(Number.prototype.valueOf,0);
					this.green = Array.apply(null, Array(256)).map(Number.prototype.valueOf,0);
					this.blue  = Array.apply(null, Array(256)).map(Number.prototype.valueOf,0);
					this.brightness  = Array.apply(null, Array(256)).map(Number.prototype.valueOf,0);
			...
			return {
				...
				update: function(opts){
					...
					for(var key in opts){
						switch(key){
							case 'red':
								redTotal += opts.red / 255 * 100;
								break;
							case 'green':
								greenTotal += opts.green / 255 * 100;
								break;
							case 'blue':
								blueTotal += opts.blue / 255 * 100;
								break;
							case 'brightness':
								brightnessTotal += opts.brightness / 255 * 100;
								break;
						}
					}
